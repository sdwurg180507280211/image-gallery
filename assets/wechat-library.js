...
function renderChips(){
  ...
  cats.forEach(cat=>{
    const b=document.createElement('button');b.type='button';b.className='wechat-chip'+(cat===active?' active':'');b.textContent=cat;
    b.onclick=()=>{active=cat;renderChips();render();};chips.append(b);
  });
}
function render(){
  const q=query.trim().toLowerCase();
  const items=COMPONENTS.filter(x=>(active==='全部'||x.category===active)&&(!q||(x.id+' '+x.name).toLowerCase().includes(q)));
  grid.replaceChildren();empty.hidden=items.length>0;
  items.forEach(item=>{
    ...
    const copyWrap=document.createElement('div');copyWrap.innerHTML='<div class="wechat-card-id"></div><div class="wechat-card-name"></div>';
    copyWrap.querySelector('.wechat-card-id').textContent=item.id;copyWrap.querySelector('.wechat-card-name').textContent=item.name;
...
function showWechat(){
  document.body.classList.add('wechat-mode');assetView.hidden=true;wechatView.hidden=false;setActiveTab('wechat');history.replaceState(null,'',location.pathname+location.search+'#wechat');
}
function showAssets(domain){
  document.body.classList.remove('wechat-mode');wechatView.hidden=true;assetView.hidden=false;setActiveTab(domain);
  if(location.hash==='#wechat')history.replaceState(null,'',location.pathname+location.search);
}
async function loadComponents(){
  const response=await fetch('./wechat/index.html',{cache:'no-store'});if(!response.ok)throw new Error(`公众号样式读取失败：HTTP ${response.status}`);
  const source=await response.text();const match=source.match(/const COMPONENTS=(\[.*?\]);const cats=/s);if(!match)throw new Error('公众号样式数据格式无法识别。');
  COMPONENTS=JSON.parse(match[1]);const count=wechatTab?.querySelector('small');if(count)count.textContent=String(COMPONENTS.length);renderChips();render();
}

document.querySelectorAll('.domain-tab').forEach(button=>{
  if(button.dataset.domain==='wechat')button.addEventListener('click',showWechat);
  else button.addEventListener('click',()=>showAssets(button.dataset.domain));
});
